...
Дата: 2026-08-01. Основано на аудите кода, не на предположениях: 65 MCP-тулов в `src/app/api/mcp/server/route.ts`, markdown-lsp (семантический поиск + граф на эмбеддингах), 43 существующих скилла в `index.json` на момент написания. *(К концу второй сессии на проде: 83 тула = 65 named + 18 webhook-регистраторов, 51 скилл. Цифры ниже по тексту — на момент написания плана.)*
...
> **Где мы сейчас (обновлено 2026-08-01, конец второй сессии).**
> **Этапы 1 и 2 выполнены и опубликованы** — каталог 43 → **51 скилл**, npm `docs-skills@1.8.22`. Разделы 5 и 5-бис реализованы. Тул `fetch_url` (T1) живёт на проде и проверен боевым вызовом; три скилла, которые он разблокировал, написаны.
> Следующая работа — **Этап 3**, вход в него: **T7 `get_page_diff_impact` → `docs-change-impact`**.
>
...
| T7 | **`get_page_diff_impact`** 👈 **СЛЕДУЮЩИЙ** | композиция `get_change_history` × `get_metric_timeseries` | «моя правка сработала?» — сейчас узнать нельзя никак | низкая (композиция существующих) |
...
| | |
|---|---|
| Каталог | **51 скилл**, 7 категорий. Режимы: audit 27 / platform 11 / authoring 8 / refactor 5 |
...
| Этап 3 | ⬜ не начат. Первый пункт — T7 `get_page_diff_impact` → `docs-change-impact` |
...
### Этап 3 — НЕ НАЧАТ — измеримость и семантика
